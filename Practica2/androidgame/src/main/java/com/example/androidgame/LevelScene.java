    ...
    public void handleInputs(){
        ...
        while (it.hasNext()) {
            ...
            switch (event.type) {
                ...
                case TOUCH_RELEASED:
                    if (this.returnButton.checkCollision(this.engine.getGraphics().realToLogicX(((AInput.TouchInputEvent)event).x),
                            this.engine.getGraphics().realToLogicY(((AInput.TouchInputEvent)event).y)))
                    {
                        ...
                        for (int i = 0; i < this.rows; ++i){
                            for (int j = 0; j < this.cols; ++j) {
                                if (this.levels[i][j].checkCollision(this.engine.getGraphics().realToLogicX(((AInput.TouchInputEvent)event).x),
                                        this.engine.getGraphics().realToLogicY(((AInput.TouchInputEvent)event).y))){
